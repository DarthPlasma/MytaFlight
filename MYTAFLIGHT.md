...
Base: Betaflight **`2026.6.1` (stable tag)**, GPLv3. Originally developed on master `14e6e1b` (2026.6.0-alpha), rebased onto the 2026.6.1 stable release on 2026-09-01. Pre-rebase branches preserved as `backup-alpha/*`.
...
# output: obj/betaflight_2026.6.1_<TARGET>.hex  -> flash via Configurator "Load Firmware [Local]"
```
...
- OSD: `OSD_BATTERY_IMPEDANCE` (ohms, "<x>.<mmm>R" e.g. 0.014R; 'R' used as ohm, and avoiding milli- since lowercase 'm'/'s' etc. are special glyphs in the OSD font), `OSD_SAG_COMP_BATT_VOLTAGE`. CLI pos: `osd_battery_impedance_pos`, `osd_sag_comp_batt_pos`.
...
- Files: `io/adsb.{c,h}` (vehicle list, distance/bearing via `GPS_distance_cm_bearing`, TTL, `findVehicleThreat()`; adapted from iNAV), hook `case MAVLINK_MSG_ID_ADSB_VEHICLE` in `telemetry/mavlink.c`, OSD elements in `osd_elements.c` (+ `osd/osd.c` PG bump), `pg/pg_ids.h` (PG_ADSB_CONFIG 2048), `cli/settings.c`, `mk/source.mk`.
- CLI: `adsb_max_dist_horiz` (m, 50000), `adsb_max_dist_vert` (m, 2000; asymmetric — hides only traffic ABOVE us beyond this, below is always shown), `adsb_detection_cone` (centideg, 2000 = ±10°), `adsb_aircraft_toa` (s, 60), `adsb_max_vehicle` (aircraft tracked at once, 5–12, default 5 — array sized to the max, all scan/count/eviction loops bounded by the runtime value; PG **not** bumped so existing adsb_* survive the flash). Own altitude uses the FUSED estimate (`GPS_home_llh.altCm + getEstimatedAltitudeCm()`).
- **Threat selection** (`findVehicleThreat()`): among tracked vehicles that are in display range, have ToA ≤ `adsb_aircraft_toa`, and whose course lies within ±`detectionCone/2` of the reciprocal bearing (i.e. heading at us), it returns the single **lowest-ToA** (most imminent) one. All the threat-driven elements below share this same vehicle.
- **Informational OSD elements** (nearest traffic): `OSD_ADSB_WARNING` (arrow-to-traffic + distance + Δalt), `OSD_ADSB_INFO` (movement arrow + class + speed + callsign), `OSD_ADSB_STATUS` ("A<detected>/<in-range>"). CLI pos: `osd_adsb_warning_pos`, `osd_adsb_info_pos`, `osd_adsb_status_pos`. **`<detected>` (x)** = every aircraft currently received over MAVLink (any distance, even without a GPS fix); **`<in-range>` (y)** = only those within `adsb_max_dist_horiz`/`adsb_max_dist_vert`. `recalculateVehicle()` deliberately does **not** evict distant traffic — filtering by distance/height is display-only — so far aircraft still count in x (the 5 tracking slots are recycled by TTL and by `adsbNewVehicle()` evicting the farthest when a new one arrives).
- **Dedicated safety OSD elements** (shown only while `findVehicleThreat()` returns a threat — decoupled from BF's generic `OSD_WARNINGS` so they can't be pre-empted and don't overwrite neighbours):
  - `OSD_ADSB_CRITICAL_WARNING` — steady **"AIRCRAFT APPROACHING &lt;n&gt;S"** (ToA in seconds), NORMAL severity/font (SEVERITY_CRITICAL selects an unpopulated font page on MSP DisplayPort → invisible). CLI pos: `osd_adsb_critical_warning_pos`.
  - `OSD_ADSB_CONE` — a two-row **collision cone**. The cone belongs to the AIRCRAFT (its approach cone); both row-2 markers are US in it. Row 1 = numeric scale `-X … 0 … +X`, X = round(`adsb_detection_cone`/2) in degrees (default ±10°; **21 cells, ~1°/cell at the default cone**). Row 2: **(a) our CURRENT position** (arrow) — column = where we sit relative to the aircraft's nose now = `-(headingError)` (always inside the cone for a threat), glyph = angle between our motion vector and the aircraft's (opposed/head-on → ▲ up, same course → ▼ down, crossing → ◄/►), or **`?`** if `imuIsHeadingValid()` false (position still valid, direction unknown). **(b) our PREDICTED position** at ToA (crosshair `SYM_AH_CENTER`) — same frame, both GPS positions projected forward (`adsbOwnProjectedConeAngleDeg()` in io/adsb.c = bearing from the aircraft's future position to ours minus the aircraft heading; equals the current marker as ToA→0; uses our GPS ground course, no compass); projected outside the cone → outward edge arrow (`SYM_ARROW_EAST/WEST`). The gap between the two markers is the CBDR cue (closing to centre = worsening, opening/leaving = resolving). Predicted marker hidden when `imuIsHeadingValid()` false, or when it lands on the current-position arrow's column (show the arrow only). Steady. Single pass: row 1 via the buffer, row-2 markers written directly (content only, no blanks — writing spaces would blank overlapping elements). CLI pos: `osd_adsb_cone_pos`. **Sign convention (left/right) internally consistent but field-verify like Feature D.**
- Adding OSD elements bumped `PG_OSD_ELEMENT_CONFIG` (→5), so **flashing a build that adds/removes ADS-B OSD elements resets all OSD element positions to defaults** — re-place the layout with the OSD tool afterwards.

### D — Vector "cruise" position hold  (**RETIRED 2026-09-01 — superseded by stock BF 2026.6.1**)

> **Upstream got there on its own**: Betaflight 2026.6.1's position hold natively commands a target **velocity** from stick deflection (`sticksSetTargetVelocity()` in `flight/autopilot_multirotor.c`: full stick = `ap_max_velocity`, body→earth rotation, feedforward, fence follows the craft, brake-to-hold on release). Exactly the CRUISE concept this fork pioneered and field-validated. Our implementation was retired at the stable rebase (no `pos_hold_navmode` any more); the alpha-era code survives on `backup-alpha/feature/poshold-vector`. History below kept for reference.
...
`python3 mytaflight-tools/build-tool.py` → http://localhost:8792. Target dropdown (~34 popular boards across DAKE/T-Motor/HolyBro/Matek/iFlight/HGLRC/Foxeer/BetaFPV/Flywoo/GEPRC/SpeedyBee/Zeez/BrainFPV — all verified to build), **Full** (everything + opt-in extras) vs **Slim** (CLOUD_BUILD, only ticked flags), custom features / sensors / receiver (radio) / telemetry (radio) / OSD SD+HD (checkboxes). Runs `make`, reports pass/fail + FLASH1/AXIM_FLASH1 %, and serves the `.hex` for download. Full mode auto-adds USE_GPS for ADS-B. Slim only passes genuinely-selectable flags (see §3 gotcha).
...
1. **Feature D**: merged to `integration`, **field-validated on DAKEFPVH743 (2026-07-06)** — behaviour correct. Remaining: **PID tuning** (esp. altitude/ALTHOLD). Optionally clamp diagonal velocity to `ap_max_velocity` (currently per-axis, ~√2 in diagonal — accepted).
