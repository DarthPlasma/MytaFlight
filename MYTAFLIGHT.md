...
- **Dedicated safety OSD elements** (shown only while `findVehicleThreat()` returns a threat — decoupled from BF's generic `OSD_WARNINGS` so they can't be pre-empted and don't overwrite neighbours):
  ...
  - `OSD_ADSB_CONE` — a two-row **collision cone**. Row 1 = numeric scale `-X … 0 … +X` where X = round(`adsb_detection_cone`/2) in degrees (default ±10°). Row 2 = a single arrow whose **column** is the threat's position in the cone (its heading error vs. the head-on collision course, 0 = dead centre; **21 cells, ~1° per cell at the default cone**, generally `X/10`°/cell) and whose **direction** is the angle between our motion vector and the aircraft's (opposed/head-on → ▲ up, same course → ▼ down, crossing → ◄/►); when our own heading isn't trustworthy (no mag and GPS course not yet acquired, `imuIsHeadingValid()` false) the arrow becomes **`?`** (position still valid, direction unknown). Row 2 also carries a second marker — a **crosshair (`SYM_AH_CENTER`)** at **our predicted position at ToA relative to the aircraft's**, both GPS positions projected forward (`adsbThreatMissAngleDeg()` in io/adsb.c; uses our GPS ground course, no compass): centre = collision, off-centre = we pass that side, and if projected outside the cone it becomes an outward edge arrow (`SYM_ARROW_EAST/WEST`) — the CBDR cue (gap to the threat arrow closing = worsening, opening = resolving). Hidden when `imuIsHeadingValid()` false, or when it lands on the threat arrow's column (show the arrow only). Steady (not blinking). Single pass: row 1 via the buffer, row-2 markers written directly (content only, no blanks — writing spaces would blank overlapping elements). CLI pos: `osd_adsb_cone_pos`.
