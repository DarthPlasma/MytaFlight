...
- **Informational OSD elements** (nearest traffic): `OSD_ADSB_WARNING` (arrow-to-traffic + distance + Δalt), `OSD_ADSB_INFO` (movement arrow + class + speed + callsign), `OSD_ADSB_STATUS` ("A<detected>/<in-range>"). CLI pos: `osd_adsb_warning_pos`, `osd_adsb_info_pos`, `osd_adsb_status_pos`.
- **Dedicated safety OSD elements** (shown only while `findVehicleThreat()` returns a threat — decoupled from BF's generic `OSD_WARNINGS` so they can't be pre-empted and don't overwrite neighbours):
  - `OSD_ADSB_CRITICAL_WARNING` — blinking **"AIRCRAFT APPROACHING &lt;s&gt;"** (ToA in seconds), CRITICAL severity. CLI pos: `osd_adsb_critical_warning_pos`.
  - `OSD_ADSB_CONE` — a two-row **collision cone**. Row 1 = numeric scale `-X … 0 … +X` where X = round(`adsb_detection_cone`/2) in degrees (default ±10°). Row 2 = a single arrow whose **column** is the threat's position in the cone (its heading error vs. the head-on collision course, 0 = dead centre; **21 cells, ~1° per cell at the default cone**, generally `X/10`°/cell) and whose **direction** is the angle between our motion vector and the aircraft's (opposed/head-on → ▲ up, same course → ▼ down, crossing → ◄/►). Steady (not blinking). CLI pos: `osd_adsb_cone_pos`.
